                    ...
                    <View>
                        {
                            doctor.image === "not available" ?
                                <View style={{display:'flex', flexDirection:'row', justifyContent:'flex-start', alignItems:'center'}}>
                                    ...
                                    />
                                    <View>
                                        <Text style={{color:'black', fontSize:20, fontWeight:'700'}}>{doctor.name}</Text>
                                        <View style={{ display: 'flex', flexDirection: 'row', justifyContent: 'flex-start', alignItems: 'center' }}>
                                            <Text style={{color:'#a3a3a3', fontSize:13}}>{doctor.age} yrs </Text>
                        ...
                        <View style={{display:'flex',flexDirection:'row', justifyContent:'space-between', alignItems:'center',  paddingVertical:3, marginVertical:6}}>
                            <View>
                                <Text selectable={true} style={{color:'black', fontWeight:'600', fontSize:18}}>{doctor.mobile}</Text>
                                <Text style={{color:Colors.primary, fontWeight:'900', fontSize:12}}>Contact Number</Text>
                            </View>
                            <View>
                                <Text selectable={true} style={{ color: 'black', fontWeight: '600', fontSize: 18 }}>{doctor.address}</Text>
                                <Text style={{color:Colors.primary, fontWeight:'900', fontSize:12, textAlign:'right'}}>Address</Text>
                            </View>
                        </View>
                        <View style={{display:'flex', flexDirection:'row', justifyContent:'space-between', alignItems:'center'}}>
                            <View>
                                <Text style={{ color: 'black', fontWeight: '600', fontSize: 18 }}>{doctor.timeofclinic.split(",")[0]}</Text>
                                <Text style={{ color: 'black', fontWeight: '600', fontSize: 18 }}>{doctor.timeofclinic.split(",")[1]}</Text>
                            </View>
                            {
